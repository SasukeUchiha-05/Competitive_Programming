...
public class BinaryTrips{
  public static long minimumTime(int[] time, int totalTrips) {
    ...
    long left = 1;
    // Maximum possible time would be min(time[i]) * totalTrips
    long right = (long) Math.min(time[0], Integer.MAX_VALUE) * totalTrips;
    ...
    while (left < right) {
        ...
        // If we can complete at least totalTrips, try a smaller time
        if (trips >= totalTrips) {
            right = mid;
        }
        // If we can't complete totalTrips, need more time
        else {
            left = mid + 1;
        }
    }
