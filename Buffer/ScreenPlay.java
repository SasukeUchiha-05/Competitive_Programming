 ...
 */
 ...
 class ScreenPlay{
     public static void main (String[] args) {
         ...
         int[] screen_plays = new int[n];
         
         for(int i=0;i<n;i++)
         {
             screen_plays[i] = sc.nextInt();
         }
         sc.close();
         
         System.out.println(screenPlay(screen_plays,n,T,V));
     }
     
     private static int screenPlay(int[] screen_plays , int n , int T , int V)
     {
         int i=0,j=n-1;
         ...
         while(i<=j){
             if(currA<screen_plays[i]){
                 refill++;
                 currA=T;
             }
             
             currA-=screen_plays[i];
             i++;
             
             if(i>j) break;
             
             if(currB<screen_plays[j]){
                 refill++;
                 currB=V;
             }
             currB-=screen_plays[j];
             j--;
         }
         return refill;
     }
