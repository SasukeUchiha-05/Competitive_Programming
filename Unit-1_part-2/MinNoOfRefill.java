...
public class MinNoOfRefill{
    public static void main(String args[])
    {
        ...
        sc.close();
        System.out.println(minNoOfRefill(guests , n , capA , capB));
    }
    
    private static int minNoOfRefill(int[] guests , int n , int capA , int capB)
    {
        int i = 0,j=n-1;
        
        int refill = 0;
        
        int currentA = capA, currentB=capB;
        
        while(i<=j)
        {
            if(currentA<guests[i])
            {
                refill++;
                currentA = capA;
